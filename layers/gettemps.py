...
class GetTemplates(object):
    ...
    def run(self,img,prefix,outfolder="./templates/"):
        # Check template directory
        regex = outfolder+prefix+'_toe?.jpg'
        paths = glob(regex)
        print(len(paths))
        self.Tnum = len(paths)
        ...
        while(1):
            ...
            if k == 27:         # esc to exit
                ...
                x,y,dx,dy = self.rect
                cut = self.mask[y:y+dy,x:x+dx]*255
                cv.imwrite(outfolder+prefix+'_toe%i.jpg'%self.Tnum, cut)
                self.Tnum +=1
